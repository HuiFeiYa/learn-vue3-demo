<template>
  <div>
    <svg width='220' version='1.1' xmlns='http://www.w3.org/2000/svg' viewBox="50 60 75 70">
      <path
         d="m 69.396426,84.364287 h 8.4836 q 3.4544,0 5.9436,2.54 2.4892,2.4892 2.4892,5.9436 v 21.132803 q 0,3.4544 -2.4892,5.9944 -2.4892,2.4892 -5.9436,2.4892 h -8.4836 q -3.4544,0 -5.9944,-2.4892 -2.4892,-2.4892 -2.4892,-5.9944 V 92.847887 q 0,-3.5052 2.4892,-5.9944 2.54,-2.4892 5.9944,-2.4892 z m 0.1016,30.327603 q 0.3048,1.7272 1.3208,2.6416 1.0668,0.9144 2.794,0.9144 1.778,0 2.9972,-1.27 1.27,-1.27 1.27,-2.9972 V 99.604287 Z m 8.3312,-22.606003 q -0.9144,-3.5052 -4.2164,-3.5052 -1.7272,0 -2.9972,1.27 -1.2192,1.2192 -1.2192,2.9972 v 14.376403 z"
         id="Number-1" class="number2" fill="none" stroke="none" />
      <path
         d="m 99.112838,92.847887 h -4.2672 q 1.016,-8.4836 8.483602,-8.4836 h 8.4836 q 3.4544,0 5.9436,2.54 2.4892,2.4892 2.4892,5.9436 v 2.0828 q 0,4.4196 -8.1788,11.430003 l -0.5588,0.4572 q -8.1788,7.0104 -8.1788,11.0236 v 0.4064 h 16.9164 v 4.2164 H 94.845638 v -4.2164 q 0,-4.4196 8.229602,-11.43 l 0.5588,-0.4572 q 8.1788,-7.010403 8.1788,-11.430003 v -2.0828 q 0,-1.778 -1.27,-2.9972 -1.27,-1.27 -2.9972,-1.27 h -4.2164 q -3.911602,0 -4.216402,4.2672 z"
         id="Number-2" class="number2" fill="none" stroke="none" />
    </svg>
  </div>
</template>

<script>
  import anime from 'animejs/lib/anime.es.js'
  export default {
    name: '',
    props:{
      isMove:{
        type:Boolean,
        default:false
      }
    },
    data() {
      return {
      }
    },
    mounted() {
      
    },
    watch:{
      isMove(newVal){
        console.log('newVal2',newVal)
        if(newVal) {
          this.draw()
        }
      }
    },
    methods: {
      draw() {
              const letterTime = 1000

      const lineDrawing = anime({
        targets: '.number2',
        // Anime 提供了 strokeDashoffset 这个属性，它会自动计算 path 的长度然后来设置 path 的 stroke-dasharry 和 stroke-dashoffset 的值
        strokeDashoffset: [anime.setDashoffset, 0],
        easing: 'easeInOutCubic',
        duration: letterTime,
        delay: function(el, i) {
          // el.setAttribute('fill','red')
          return letterTime * i
        },
        begin: function(anim) {
          const letters = document.querySelectorAll('.number2')
          let i
          for (i = 0; i < letters.length; ++i) {
            letters[i].setAttribute('stroke', '#373B44')
            letters[i].setAttribute('stroke-width', 2)
            document.getElementById('index1').setAttribute('fill','none')
            document.getElementById('index2').setAttribute('fill','none')
          }
        },
        update: function(anim) {
          if(anim.currentTime >= 1 * letterTime) {
            document.getElementById('Number-1').setAttribute('fill','#ee9ca7')
          }
          if(anim.currentTime >= 2 *letterTime) {
            document.getElementById('Number-2').setAttribute('fill','#ee9ca7')
          }
        },
        loop:true,
        direction: 'alternate'
      })
      lineDrawing.restart()
      }
    }
  }
</script>

<style lang='less' scoped>

</style>
