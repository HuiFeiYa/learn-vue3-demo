<template>
    <div class="line-draw">
      ...
      <button class="play-drawing" @click="onClick">isux</button>
    </div>
</template>
<script lang="ts">
...
export default class Slide extends Vue {
  mounted() {
    // const letter1 = this.$refs.letter1
    // console.log('letter1',letter1)
    // (document.querySelector(".letter-i") as any).setAttribute("fill", "#fff");
  }
  onClick() {
var letterTime = 2000
    ...
    if (anim.currentTime >= 2 * letterTime) {
      (document.querySelector(".letter-s") as any).setAttribute("fill", "#fff");
    }
    if (anim.currentTime >= 3 * letterTime) {
      (document.querySelector(".letter-u") as any).setAttribute("fill", "#fff");
    }
    if (anim.currentTime >= 4 * letterTime) {
      (document.querySelector(".letter-x") as any).setAttribute("fill", "#fff");
    }
        },
